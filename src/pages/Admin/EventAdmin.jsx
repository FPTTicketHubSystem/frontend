...
import { GetRateByEventIdService, UpdateRatingStatusService, DeleteRatingService } from '../../services/EventRatingService';
...
const EventAdmin = () => {
  ...
  try {
    ...
    showSuccessToast(`Event status changed to ${newStatus} successfully`);
  } catch (error) {
...
const handleRatingStatusChange = async (ratingId, newStatus) => {
  try {
    ...
    fetchRatingsByEventId(currentEvent.eventId);
    showSuccessToast('Rating status updated successfully');
  } catch (error) {
...
const filteredRatings = eventRatings.filter((rating) => {
  return (
    (ratingFilterStatus === 'All' || rating.status.trim() === ratingFilterStatus) &&
    (rating.accountName.toLowerCase().includes(ratingSearchTerm.toLowerCase()) ||
  ...
  const columns = [
    ...
    { title: 'Thời gian bắt đầu', dataIndex: 'startTime', key: 'startTime', render: (text, event) => formatDate(event.startTime) },
    { title: 'Thời gian kết thúc', dataIndex: 'endTime', key: 'endTime', render: (text, event) => formatDate(event.endTime) },
    { title: 'Hành động', key: 'actions', render: (text, event) => (
      <div className="button-container">
        <button onClick={() => handleEventClick(event)} className="btn-tooltip btn btn-outline-primary btn-sm" title="Chi tiết" style={{ marginRight: '10px' }}>
          <i className="icon-eye"></i>
        </button>
        <ChangeStatusButton 
          eventId={event.eventId}
          currentStatus={event.status} 
          onStatusChange={(newStatus) => handleChangeEventStatus(event.eventId, newStatus)} 
          onSuccess={(newStatus) => showSuccessToast(`Event status changed to ${newStatus} successfully`)}
                        ...
                        <tbody>
                          {currentEvents.map((event) => (
                            <tr key={event.eventId}>
                              {columns.map((column) => (
                                <td key={column.key}>
                                  {column.render ? column.render(null, event) : event[column.dataIndex]}
                                </td>
                              ))}
                            </tr>
                          ))}
            ...
            <ul className="nav nav-tabs1">
              ...
              <div className="event-info">
                <div className="event-image">
                  <img src={currentEvent.themeImage} alt={`Event picture of ${currentEvent.eventName}`} />
                </div>
                <div className="event-details">
                  <p><strong>Organizer:</strong> {currentEvent.fullName}</p>
                  <p><strong>Category:</strong> {currentEvent.categoryName}</p>
                  <p><strong>Location:</strong> {currentEvent.location}</p>
                  <p><strong>Address:</strong> {currentEvent.address}</p>
                  <p><strong>Start Time:</strong> {formatDate(currentEvent.startTime)}</p>
                  <p><strong>End Time:</strong> {formatDate(currentEvent.endTime)}</p>
                </div>
              </div>
              
              <div className="event-description">
                <h3>Description</h3>
                <div dangerouslySetInnerHTML={{ __html: currentEvent.eventDescription }} />
              </div>
                ...
                <div className="ratings-tab">
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <div className="btn-group">
                      <button
                        className={`btn ${ratingFilterStatus === 'All' ? 'btn-primary' : 'btn-outline-primary'}`}
                        onClick={() => setRatingFilterStatus('All')}
                      >
                        All
                      </button>
                      <button
                        className={`btn ${ratingFilterStatus === 'Active' ? 'btn-primary' : 'btn-outline-primary'}`}
                        onClick={() => setRatingFilterStatus('Active')}
                      >
                        Active
                      </button>
                      <button
                        className={`btn ${ratingFilterStatus === 'Pending' ? 'btn-primary' : 'btn-outline-primary'}`}
                    ...
                    <tbody>
                      {currentRatings.map((rating) => (
                        <tr key={rating.eventRatingId}>
                          ...
                          <td>{rating.rating}<i className="icon-star1"></i></td>
                          <td>{rating.review}</td>
                          <td>{formatDate(rating.ratingDate)}</td>
                          <td>
                            <select
                              value={rating.status.trim()}
                              onChange={(e) => handleRatingStatusChange(rating.eventRatingId, e.target.value)}
                            >
                          ...
                          <td>
                            <button 
                              onClick={() => handleDeleteRating(rating.eventRatingId)} 
                  ...
                  <div className="d-flex justify-content-center">
                    <nav>
                      <ul className="pagination">
                        <li className={`page-item ${ratingCurrentPage === 1 ? 'disabled' : ''}`}>
                          <button 
                            className="page-link" 
                            onClick={() => setRatingCurrentPage(ratingCurrentPage - 1)}
                          >
                            Previous
                          </button>
                        </li>
                        {[...Array(Math.ceil(filteredRatings.length / ratingsPerPage)).keys()].map((number) => (
                          <li 
                            key={number} 
                        ...
                        <li className={`page-item ${ratingCurrentPage === Math.ceil(filteredRatings.length / ratingsPerPage) ? 'disabled' : ''}`}>
                          <button 
                            className="page-link" 
                            onClick={() => setRatingCurrentPage(ratingCurrentPage + 1)}
                          >
